...
export default function Navigation({ variant = 'marketing' }: NavigationProps) {
  ...
  return (
    <header className="border-b border-brand-light/40 bg-white/80 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
        <Link href="/" className="flex items-center gap-2 text-xl font-heading text-brand-dark">
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-brand text-white">FF</span>
          <span>FeedbackFlow</span>
        </Link>
        <nav className="flex flex-wrap items-center gap-4 text-sm font-medium text-subtle">
          {links.map((link) => (
